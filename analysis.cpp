...
inline void add_swap_scores(int & score0, int & score1, int add0 = 0) {
    ...
    if (score0 > 1 && score1 > 1 && (score0 % score1 == 0 || score1 % score0 == 0)) swap(score0, score1);
}

// *** Implementation of LearningStrategy class ***

void LearningStrategy::learn(IStrategy & oppo_strat, int number,
    pair<int, int>focus, volatile int * interrupt, bool quiet, int announce_interval, int wr_interval) {
        MatrixStrategy testStrat = MatrixStrategy(*this);

        if (!quiet) {
            cout << "Learning procedure started.\n" << "Initial win rate: ";
            cout << average_win_rate(testStrat, oppo_strat, -1, 0, 0, 0) << "\n\n";
        }
        ...
        for (int i = 0; i < number; ++i) {
            ...
            if (!quiet) {
                if (i % announce_interval == announce_interval - 1) {
                    cout << i + 1 << " round" << (i ? "s" : "") << " of learning completed, " << number - i - 1 << " remaining. \n";
                }

                if (i % wr_interval == wr_interval - 1) {
                    cout << "Updated Win rate @ " << i + 1 << ": " << best_awr << "\n\n";
                }
            }
        ...
        if (!quiet) {
            ...
            if (interrupt && *interrupt) {
                ...
                cout << "Final win rate: " << average_win_rate(testStrat, oppo_strat, -1, 0, 0, 0) << endl;
            }
        }
}

pair<int, int> LearningStrategy::next_focus(pair<int, int> focus) {
    // advance to the next value of focus. will cycle through all possible values of focus

    if (focus.first < focus.second) {
        swap(focus.first, focus.second);
    }

    else {
        if (focus.first > focus.second) swap(focus.first, focus.second);
    ...
    for (int i = 0; i < MAX_TURNS; ++i) {
        ...
        for (int j = 0; j < GOAL; ++j) {
            for (int k = 0; k < GOAL; ++k) {
                ...
                for (int s = 1; s < min(GOAL - j, DICE_SIDES * MAX_ROLLS + 1); ++s) {
                    if (j + s >= GOAL) break;
...
pair<double, int> compute_win_rates(int i, int j, int trot) {
    ...
    if (win_rate_at_score[i][j][trot].second == -1) {
        ...
        win_rate_at_score[i][j][trot] = pair<double, int>(best_wr, best_strat);
    }
...
MatrixStrategy * create_final_strat(bool quiet) {
    ...
    if (!quiet) {
        ...
        cout << "Preparing 0/2, please wait ..." << endl;
    }
    ...
    if (!quiet) cout << "Preparing 1/2, please wait ..." << endl;
...
void round_robin_coroutine(vector<pair<string, IStrategy *> > strats,
    ...
    for (unsigned i = 0; i < N; ++i){
        ...
        for (unsigned j = i + 1 + jbase; j < N; j += jdelta){
            ...
            string name0 = strats[i].first, name1 = strats[j].first;
...
vector<pair<int, string> *> round_robin(vector<pair<string, IStrategy *> > strats,
    ...
    for (int i = 0; i < threads; ++i) {
        thread * th = new thread(round_robin_coroutine,
...
void draw_strategy_diagram(IStrategy & strat) {
    cout << "Y-axis is player score, X-axis is opponent score. Bottom left is 0, 0.\n" << endl;
